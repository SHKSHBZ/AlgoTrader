...
class ZerodhaLiveAPI:
    ...
    def authenticate(self, api_key: str, api_secret: str = None):
        ...
        if self._load_existing_session():
            print(f"[AUTH] Using existing session")
            return True
    ...
    def _load_existing_session(self) -> bool:
        ...
        try:
            ...
            session_date = datetime.fromisoformat(session_data['created_at']).date()
            today = datetime.now().date()
            
            if session_date != today:
                print(f"[AUTH] Session expired (created: {session_date})")
                return False
...
class PerfectTraderPaperTrading:
    ...
    def __init__(self, initial_capital: float = 250000, use_live_data: bool = True, api_key: str = None, force_fresh_start: bool = False):
        ...
        # Initialize live API if requested
        if use_live_data and api_key:
            self.live_api = ZerodhaLiveAPI()
            try:
                print(f"[LIVE] Initializing Zerodha connection...")
                
                # Check if api_key contains both key and secret
                if ',' in api_key:
                    key, secret = api_key.split(',', 1)
                    key, secret = key.strip(), secret.strip()
                else:
                    key = api_key
                    secret = input("[AUTH] Enter your API Secret: ").strip()
                
                if self.live_api.authenticate(key, secret):
                    if self.live_api.load_instruments():
                        print(f"[LIVE] ✅ Zerodha integration successful!")
                        
                        # Show user info
                        profile = self.live_api.get_profile()
                        if profile:
                            print(f"[USER] Logged in as: {profile.get('user_name', 'Unknown')}")
                            print(f"[USER] Broker: {profile.get('broker', 'ZERODHA')}")
                    else:
                ...
                print(f"[LIVE] ❌ Zerodha connection failed: {e}")
                print(f"[LIVE] Falling back to cached data")
                self.use_live_data = False
        elif use_live_data:
            print(f"[LIVE] No API credentials provided")
            print(f"[LIVE] Get your API key and secret from: https://kite.trade/")
            print(f"[LIVE] Using cached data for now")
            self.use_live_data = False
            ...
            if self.use_live_data and self.live_api:
                ...
                if (cache_key in self.price_cache and 
                    ...
                    if live_price > 0:
                        ...
                        print(f"[ERROR] Failed to get live price for {symbol}, trying Yahoo Finance")
                        price = self._get_yahoo_price(symbol)
            else:
                # No Zerodha API, try Yahoo Finance during market hours
                print(f"[LIVE] No Zerodha API, trying Yahoo Finance for {symbol}")
                price = self._get_yahoo_price(symbol)
    ...
    def _get_yahoo_price(self, symbol: str) -> float:
        """Get live price from Yahoo Finance as backup"""
        try:
            ...
            if not data.empty:
                ...
                print(f"[YAHOO] {symbol}: Rs.{price:.2f} (Live)")
...
def main():
    """Main paper trading function with REAL Zerodha integration"""
    
    # ====== CONFIGURATION ======
    USE_LIVE_DATA = True  # Set to False for cached data only
    
    # Interactive mode - will ask for credentials if you want to test
    ASK_FOR_CREDENTIALS = True  # Set to False to skip interactive input
    
    # Option 1: Set both credentials here (will override interactive mode)
    ZERODHA_API_KEY = None    # Your API key from kite.trade
    ZERODHA_API_SECRET = None # Your API secret from kite.trade
    
    # Option 2: Set as single string (key,secret)
    # ZERODHA_CREDENTIALS = "your_api_key,your_api_secret"  
    ZERODHA_CREDENTIALS = None
    
    print("=" * 60)
    print("[CONFIG] PERFECT TRADER - LIVE ZERODHA INTEGRATION")
    print("=" * 60)
    print(f"[CONFIG] Live Data: {'Enabled' if USE_LIVE_DATA else 'Disabled'}")
    
    # Determine credentials
    if ZERODHA_CREDENTIALS:
        api_creds = ZERODHA_CREDENTIALS
        print(f"[CONFIG] Zerodha: Credentials provided")
    elif ZERODHA_API_KEY:
        api_creds = ZERODHA_API_KEY
        print(f"[CONFIG] Zerodha: API Key provided")
    else:
        api_creds = None
        print(f"[CONFIG] Zerodha: No credentials set")
    
    print("=" * 60)
    
    # Interactive credential input for testing
    if USE_LIVE_DATA and not api_creds and ASK_FOR_CREDENTIALS:
        print("[INTERACTIVE] Would you like to test Zerodha integration?")
        print("[INTERACTIVE] You'll need your API Key and Secret from https://kite.trade/")
        response = input("[INTERACTIVE] Enter 'y' to proceed with Zerodha, or 'n' for cached data: ").strip().lower()
        
        if response == 'y':
            api_key = input("[INPUT] Enter your Zerodha API Key: ").strip()
            api_secret = input("[INPUT] Enter your Zerodha API Secret: ").strip()
            
            if api_key and api_secret:
                api_creds = f"{api_key},{api_secret}"
                print(f"[INTERACTIVE] ✅ Credentials provided!")
            else:
                print(f"[INTERACTIVE] ❌ Invalid credentials, using cached data")
        else:
            print(f"[INTERACTIVE] Using cached data as requested")
        
        print("-" * 60)
    elif USE_LIVE_DATA and not api_creds:
        print("[INFO] To enable REAL-TIME Zerodha prices:")
        print("[INFO] 1. Get API credentials from: https://kite.trade/")
        print("[INFO] 2. Set ZERODHA_API_KEY and ZERODHA_API_SECRET above")
        print("[INFO] 3. Or set ASK_FOR_CREDENTIALS = True for interactive input")
        print("-" * 60)
        print("[FALLBACK] Using cached data for now...")
        print("-" * 60)
    
    # Initialize engine with live Zerodha support
    engine = PerfectTraderPaperTrading(
        initial_capital=250000,
        use_live_data=USE_LIVE_DATA,
        api_key=api_creds,
        force_fresh_start=True  # Always start fresh with Rs.2,50,000
    )
